<template>
  <div class="min-h-screen relative overflow-hidden">
    ...
    <div
        class="min-h-screen bg-gradient-to-b from-[var(--color-medium)] via-[var(--color-light)] to-[var(--color-primary)] text-[var(--color-dark)] p-6 relative z-0"
    >
      ...
      <div class="relative z-10">
        ...
        <h1 class="text-4xl font-bold mb-10 text-left text-white drop-shadow-md">
          Recent Projects
        </h1>

        <hr class="border-t-2 border-white mb-10 opacity-50"/>

        <div
            class="grid gap-6"
            style="grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); grid-auto-rows: minmax(200px, auto);"
        >
          <RouterLink
...
.min-h-screen {
  background: linear-gradient(to bottom, var(--color-medium), var(--color-light), var(--color-primary));
}
...
/* Animações das ondas - CORRIGIDAS */
.wave-animation-slow {
...
/* Keyframes corrigidos para movimento real das ondas */
...
@keyframes waveMoveMedium {
  ...
  25% {
    transform: translateX(-3%) scaleX(1.03) translateY(-1%);
  }
  50% {
    transform: translateX(2%) scaleX(0.97) translateY(0.5%);
  }
  75% {
    transform: translateX(4%) scaleX(1.02) translateY(-0.5%);
  }
}
...
@keyframes waveMoveFast {
  ...
  25% {
    transform: translateX(-5%) scaleX(1.05) scaleY(1.02);
  }
  50% {
    transform: translateX(3%) scaleX(0.95) scaleY(0.98);
  }
  75% {
    transform: translateX(6%) scaleX(1.03) scaleY(1.01);
  }
}

/* Efeito de ondulação mais pronunciado */
.wave-container svg {
...
/* Efeito de profundidade com filtros */
.wave-animation-slow svg {
  filter: blur(0.5px);
}

.wave-animation-medium svg {
  filter: blur(0.3px);
}

.wave-animation-fast svg {
  filter: blur(0.1px);
}

/* Gradiente sutil no fundo para complementar */
.min-h-screen::before {
  content: '';
  position: absolute;
  top: 0;
